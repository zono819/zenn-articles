...
```mermaid
sequenceDiagram
    participant C as 別エージェント
    participant S as ハザードスコア API
    participant M as MLIT MCP / API
    C->>S: GET /.well-known/agent-card.json（A2A 発見）
    S-->>C: AgentCard + skill 一覧
    C->>S: POST /skills/hazard-score（未払い）
    S-->>C: 402 Payment Required
    Note over C: USDC 0.01 を署名
...
- **API キー申請**: [不動産情報ライブラリ API 申請ページ](https://www.reinfolib.mlit.go.jp/api/request/)
...
flowchart TD
    ...
    subgraph API ["ハザードスコア API サーバー（本記事で作るもの）"]
    ...
    MLIT["MLIT 不動産情報ライブラリ API\n（無料・国交省公式）\n洪水 / 津波 / 土砂 / 地価"]

    CA -->|"1. GET /.well-known/agent-card.json"| AC
    CA -->|"2. skill 呼び出し"| A2A
    A2A --> X402
    X402 --> MC
    MC -->|"3. MLIT API 呼び出し"| MLIT
```
...
sequenceDiagram
    ...
    C->>S: POST /a2a
    S-->>C: 402 Payment Required + 支払い条件
    Note over C: USDC 0.001 を署名
    C->>S: POST /a2a + X-PAYMENT ヘッダー
    S->>BC: 決済を検証
    BC-->>S: 検証 OK
    S->>M: データ取得
    M-->>S: 生データ
...
x402 日本語解説:
@[card](https://zenn.dev/aki_on/articles/498effc7c4f18b)

MLIT MCP 試してみた:
@[card](https://zenn.dev/rescuenow/articles/fe5fc6f226cea7)

エージェント決済アーキテクチャ分析（MoonAgents Card）:
...
ArXiv 論文（A2A + x402）:
@[card](https://arxiv.org/html/2507.19550v1)
