...
一般チャット LLM がブラウジングツールを持っていても、「どのドメインの AgentCard を取りに行くか」をユーザーに聞かずに決める仕組みがない。これはツールの有無ではなく、自律的な目標分解とサービス探索エンジンがあるかどうかの問題だ。

さらに根本的な課題は、**AgentCard のインデックスが存在しない**ことだ。Google がウェブページを発見できるのは、クローラーがリンクをたどり PageRank でページの重要度を評価するインデックスを構築しているからだ。AgentCard は `/.well-known/` という標準的な置き場所を RFC 8615 で定義したが、「どのドメインに AgentCard が存在するか」を横断検索できるインデックスサービスは 2026 年 5 月時点で存在しない。ドメインを知っていれば取得できるが、ドメイン自体をどう発見するかは未解決のままだ。
...
### 無料 AgentCard と有料 AgentCard の違い

すべての AgentCard が有料とは限らない。`x402` フィールドを持たないスキルは無料で呼び出せる。ブラウジングツールを持つ一般チャット LLM でも、URL を手動指定すれば無料スキルを今すぐ呼び出せる（壁①・②の制約は残る）。

決済の壁が問題になるのは有料コンテンツに限られる。その場合の近道として 2 つのモデルが考えられる：

1. **LLM プロバイダー前払いモデル**: OpenAI の Prepaid Credits や Anthropic の API クレジットのように、LLM プロバイダーがユーザーの代わりに API コストを仲介する構造。プロバイダー側が AgentCard サービスへの決済を肩代わりすれば、エンドユーザーはウォレットを持たなくても有料コンテンツにアクセスできる。AP2 の設計で検討されているモデルの一つ。

2. **ユーザーウォレット紐付け**: ユーザーが自分のウォレット（OWS 等）をセッションに一度紐付けることで、LLM が間接的に x402 署名を実行できる構造。A2A の Extended AgentCard と組み合わせると、ポリシーゲート付きの委譲署名として機能する。
...
| AgentCard を自律的に発見 | ❌ インデックスが存在しない | ✅ サービスカタログ経由 |
| 無料スキルを呼び出す（x402 なし） | ✅ 手動 URL 指定なら可能 | ✅ |
| 402 を受けて USDC 署名・再送 | ❌ ウォレット非保有 | ✅ OWS 経由 |
| LLM プロバイダー前払いで有料 API 利用 | 🔄 AP2 で検討中 | 🔄 実装中 |
| taskId を跨いだ状態管理 | ❌ | ✅ |
...
なお、`x402` フィールドを持たない**無料の AgentCard** については決済の壁は存在せず、ブラウジングツールを持つ LLM が URL を手動指定して呼び出すことは現状でも技術的に可能だ。壁③が問題になるのは有料コンテンツに限られる。
